...
class TrajectoryGen: 
    ...
    def __init__(self, ROM, desiredVel, reps):
        self.ROM = ROM 
        self.desiredVel = desiredVel 
        self.reps = reps 
    def initTaskSpaceCurve(self): 
        x = np.arange(-1,0,0.1)
        y = -300*(x**4) 
        points = np.c_[y,x]
    ...
    def analyticIK(self,x,y,l = 1): 
        theta = math.atan2(y,x) 
        if theta == 0: 
            d = x
        else:
            d = x/(math.cos(theta)) - l
        jointStates = np.array([theta,d]) 
        return jointStates
        
    def initJointSpaceCurve(self):
        jointStates = np.empty([1,2])
        taskSpacePath = self.initTaskSpaceCurve()
        for i in range(taskSpacePath.shape[0]):
            x = taskSpacePath[i][0] 
            y = taskSpacePath[i][1]

            currentJointStates = self.analyticIK(x,y) 

            if i == 0: 
                jointStates = currentJointStates 
            else: 
                jointStates = np.c_[jointStates,currentJointStates]
        return jointStates.transpose()


traj = TrajectoryGen(45, 1, 10)
t = traj.initJointSpaceCurve()
print(traj.reps)
print(t)
print(t.shape[0])
...
dur = 10000 
psg = np.linspace(0,dur,t.shape[0])
vel = np.array([0,0])
acc = np.array([0,0])
...
theta = traj[:,0]
d = traj[:,1]
l = np.linspace(0,10000,traj.shape[0])


#plt.title("Matplotlib demo") 
#plt.xlabel("t axis caption") 
#plt.ylabel("theta axis caption")
#plt.plot(l.transpose(),theta) 
#
#
#plt.title("Matplotlib demo") 
#plt.xlabel("t axis caption") 
#plt.ylabel("d axis caption") 
#plt.plot(l.transpose(),d) 
#plt.show()
